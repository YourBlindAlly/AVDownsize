...
To change settings, click the Settings button in the compression menu. Settings include the FFmpeg path, output folder, output file suffix, 4K downscaling, automatic deletion of the original file, and whether to show a completion summary. Uncheck the completion summary option to enable silent mode, where the menu closes after OK and no further windows appear.
